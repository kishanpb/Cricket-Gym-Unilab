"""Predeclare and evaluate the smooth bounded arm-action learning experiment."""
...
DIRECTORY = ROOT / "g1_cricket_results/tanh_v1"
...
def validate_config(saved, reference):
    ...
    expected["env"]["actions"]["residual"]["_target_"] = (
        "unilab.tasks.manipulation.g1_cricket.tanh_residual.TanhPriorResidualCfg"
    )
    expected["training"]["log_dir"] = "g1_cricket_results/tanh_v1/right"
    if saved != expected:
        raise ValueError("tanh config differs beyond residual mapping and log directory")
    assert saved["algo"]["resume"] is False and saved["algo"]["resume_path"] is None


def preflight():
    if (DIRECTORY / "preflight.json").exists() or RUN_DIR.exists():
        raise FileExistsError("tanh experiment already has retained inputs or a run")
    ...
    closed_path = ROOT / "g1_cricket_results/swing_v1/trained_evaluation.json"
    closed = json.loads(closed_path.read_text())
    check_hashes(closed["input_sha256"])
    assert not any(row["passed"] for report in closed["reports"] for row in report["rows"])
    sources = dict(parent["reports"][0]["source_sha256"])
    for name in (
        "src/unilab/tasks/manipulation/g1_cricket/tanh_residual.py",
        "src/unilab/conf/ppo/task/g1_cricket_tanh_v1/mujoco.yaml",
        "docs/g1_cricket_tanh_v1.md",
        "scripts/evaluate_g1_cricket_tanh.py",
    ...
    with initialize_config_dir(config_dir=str(ROOT / "src/unilab/conf/ppo"), version_base="1.3"):
        config = OmegaConf.to_container(
            compose(
                "config",
                overrides=[
                    "task=g1_cricket_tanh_v1/mujoco",
    ...
    result = {
        "scope": "smooth_arm_residual_pretraining_contract",
        "config": config,
    ...
    print("Action-only composed config and parent/source hashes verified before training.")
...
def run():
    ...
    assert summary["last_checkpoint"] == "g1_cricket_results/tanh_v1/right/model_2079.pt"
    pinned.update(
        {
            str(path.relative_to(ROOT)): sha256(path)
            for path in (
    ...
    for index, dt in enumerate((0.00025, 0.000125)):
        ...
        for name in ("run_config", "run_summary"):
            assert (
                report[f"{name}_sha256"] == pinned[f"g1_cricket_results/tanh_v1/right/{name}.json"]
            )
        ...
        validate_baseline(report, parent["reports"][index], same_reward=True)
        report["source_sha256"].update(inputs["source_sha256"])
        report["scope"] = "fresh_right_ppo_smooth_arm_residual_development_not_promoted"
    ...
    result = {
        "scope": "fresh_ppo_smooth_arm_residual_complete_development_evaluation",
        "input_sha256": pinned,
        "reports": reports,
